...
public class BigramAnalyzer {
  ...
  /**
   * INDEPENDENT Bigram analysis - completely separate from individual movement check NOW: Only runs
   * for EVEN-length sequences and detects specific swap patterns
   */
  public Set<String> analyze(List<List<String>> sequences, Set<String> all) {
    ...
    // Check if this is a true swap (no tokens in same position)
    ...
    if (!isTrueSwap) {
      return contentFromBigrams; // Not a swap, just optional tokens
    }
    ...
    if (secondBigramSeq1.equals(firstBigramSeq2) && firstBigramSeq1.equals(secondBigramSeq2)) {
      isSwapPattern = true;
    } else if (firstBigramSeq1.equals(secondBigramSeq2)
        && secondBigramSeq1.equals(firstBigramSeq2)) {
      isSwapPattern = true;
    }

    if (isSwapPattern) {
      // For true swap patterns, second token of each moving bigram is content
      // First token might be dual (both structural and content), but be conservative

      if (firstBigramSeq1.size() == 2) {
        String secondToken = firstBigramSeq1.get(1); // Second token is content
        if (all.contains(secondToken)) {
          contentFromBigrams.add(secondToken);
        }
      }

      if (secondBigramSeq1.size() == 2) {
        String secondToken = secondBigramSeq1.get(1); // Second token is content
        if (all.contains(secondToken)) {
          contentFromBigrams.add(secondToken);
        }
      }

      // We don't mark any dual here letting the main analysis decide
    }
