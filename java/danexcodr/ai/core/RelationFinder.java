...
public class RelationFinder {
  ...
  public Set<String> findConnectors(String token) {
    Set<String> connectors = new HashSet<String>();

    List<Content> patterns = patternProcessor.findAllContentsWithTerm(token);
    for (Content rp : patterns) {
      if (rp.getT1().equals(token)) {
        connectors.add(rp.getT2());
      } else if (rp.getT2().equals(token)) {
  ...
  public void validateInferences(
      String term1, String term2, List<List<String>> generatedSequences) {
